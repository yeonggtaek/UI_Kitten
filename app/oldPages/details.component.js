...
const StarIcons = (props): IconElement => <Icon {...props} name="stars" />;
...
const Header = (props: ViewProps): React.ReactElement => (
  <View {...props}>
    <Text category="h6">Jujutsu Kaisen</Text>
    <Text category="s1">By Gege Akutami</Text>
  </View>
);
...
const Footer = (props: ViewProps): React.ReactElement => (
  ...
  >
    <Button style={cardStyles.footerControl} size="small" status="basic">
      Deny
    </Button>
...
export const DetailsScreen = ({ navigation }) => {
  const [myselectedIndex, setMySelectedIndex] = React.useState(
    new IndexPath(0)
  );
  ...
  const renderItem = ({
    ...
    item: { title: string },
    index: number,
  }): React.ReactElement => <ListItem title={`${item.title} ${index + 1}`} />;
  ...
  const renderShakeIcon = (props): IconElement => (
    <Icon {...props} ref={shakeIconRefpress} animation="shake" name="shake" />
  );
    ...
    <ScrollView>
      ...
      >
        ...
        <View style={{ flexDirection: "row", gap: 8 }}>
          ...
          <Text
            style={{ borderRadius: 4, padding: 2, backgroundColor: "#3366FF" }}
            status="control"
          >
            Control your self
          </Text>
        ...
        >
          ...
          >
            <SelectItem
              accessoryLeft={HeartIcon}
              title="Click here for free food!!!"
            />
        ...
        <View style={{ gap: 10 }}>
          <Card>
            <Text>
              Bee Movie By Jerry Seinfeld NARRATOR: (Black screen with text; The
              sound of buzzing bees can be heard) According to all known laws of
              aviation, : there is no way a bee should be able to fly. : Its
              wings are too small to get its fat little body off the ground. :
              The bee, of course, flies anyway : because bees don't care what
              humans think is impossible. BARRY BENSON: (Barry is picking out a
              shirt) Yellow, black. Yellow, black. Yellow, black. Yellow, black.
              : Ooh, black and yellow! Let's shake it up a little. JANET BENSON:
              Barry! Breakfast is ready! BARRY: Coming! : Hang on a second.
              (Barry uses his antenna like a phone) : Hello? ADAM FLAYMAN:
              (Through phone) - Barry? BARRY: - Adam? ADAM: - Can you believe
              this is happening? BARRY: - I can't. I'll pick you up. (Barry
        ...
        <Card style={cardStyles.card} header={Header} footer={Footer}>
          <Text>
            ...
            Gege Akutami. It was serialized in Shueisha's shōnen manga magazine
            Weekly Shōnen Jump from March 2018 to September 2024, with its
            chapters collected in 28 tankōbon volumes as of October 2024.
          </Text>
        </Card>
        ...
        <View style={bananaStyles.container}>
          <Button
            style={{ margin: 2 }}
            status="primary"
            accessoryLeft={StarIcon}
